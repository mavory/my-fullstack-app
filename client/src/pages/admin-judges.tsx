...
export default function AdminJudges() {
  ...
  const [dialogRole, setDialogRole] = useState<"judge" | "admin">("judge");
  ...
  const handleCreateUser = (data: UserForm) => {
    createUserMutation.mutate({ ...data, role: dialogRole });
  };

  const handleEditUser = (user: UserType) => {
    setEditingUser(user);
    setDialogRole(user.role === "admin" ? "admin" : "judge");
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Dialog open={isCreateDialogOpen || !!editingUser} onOpenChange={(open) => {
                ...
                <DialogTrigger asChild>
                  <Button size="sm" onClick={() => { setIsCreateDialogOpen(true); setDialogRole(role as "judge" | "admin"); }}>
